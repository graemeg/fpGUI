{ Here we show added some transformation - rotation and scaling. We also show
  that we actually have two pipelines. For for the fill, and one for the stoke. }
program example_pipeline6;
...
begin
  ...
  matrix.scale(0.4, 0.75);
  matrix.translate(FImage.Width/2, FImage.Height/2);

  curve.Construct(@path);  // generates the curve line segments
  trans_curve.Construct(@curve, @matrix);  // apply transformation against the curve
