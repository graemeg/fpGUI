...
uses
  ...
  agg_conv_curve,
  agg_font_freetype,  // FreeType 2 external library
...
begin
  ...
  if font_file = '' then
  begin
    WriteLn('Usage: test_ft2 <font_file.ttf> [font_size] [text] [output_file]');
    ...
    WriteLn('  output_file - Output PNG file (default: output_ft2.png)');
    WriteLn;
    WriteLn('Example: test_ft2 /usr/share/fonts/truetype/dejavu/DejaVuSans.ttf 24 "Hello World"');
    Halt(1);
  end;
...
begin
  ...
  // Initialize font engine (FreeType 2 external library)
  feng.Construct;
  fman.Construct(@feng);
  curves.Construct(fman.path_adaptor);
  curves.approximation_scale_(2.0);

  WriteLn('Using FreeType 2 external library');
end;
...
begin
  try
    WriteLn('FreeType 2 Library Font Rendering Test');
    WriteLn('=======================================');
    WriteLn;
    ...
    WriteLn;
    WriteLn('Done! Compare this output with FreeType 1 output.');
