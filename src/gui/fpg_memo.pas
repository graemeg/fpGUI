...
begin
  // horizontal adjust
  RecalcLongestLine;
  tw := GetCursorX - FSideMargin;

  if tw > VisibleWidth - 2 then
    inc(FDrawOffset, tw - VisibleWidth + 2)
  else if tw < 0 then
  begin
    inc(FDrawOffset, tw);
    if FDrawOffset > 2 then
      Dec(FDrawOffset, 2)
    else
      FDrawOffset:=0;
  end;
...
var
  ...
  yp: Integer;
begin
  tw := TabbedTextWidth(UTF8Copy(GetLineText(ALine), 1, APosition));
