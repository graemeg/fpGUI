...
type
  ...
  private
    ...
    function    TabbedTextWidth(const AText: TfpgString): integer;
...
begin
  ...
  begin
    lw := TabbedTextWidth(getlinetext(n));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        // drawing cursor
        tw := TabbedTextWidth(UTF8Copy(ls, 1, FCursorPos));
...
begin
  ...
  FMouseDragging := True;
  FMouseDragPos  := CurPosByX(x, GetLineText(lnum));
  FCursorPos     := FMouseDragPos;
  FCursorLine    := lnum;

  if (ssShift in shiftstate) then
  begin
    FSelEndLine := lnum;
    FSelEndpos  := FMouseDragPos;
    FSelecting := True;
  end
  else
  begin
...
begin
  ...
  if lnum > LineCount-1 then
    lnum := LineCount-1;
  cp := CurPosByX(x, GetLineText(lnum));
